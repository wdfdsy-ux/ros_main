 ...
 *
 * 机械臂移动序列:
 *   Step 1: 自动启动 split.py (YOLO 检测)
 *   Step 2: 等待检测结果
 *   Step 3~4: 获取坐标 → 接近物块上方
 *   Step 5: 下降抓取 (30mm)
 *   Step 6~7: 吸盘吸取 → 抬起
 *   Step 8: 放置 → 释放
 *   Step 9: 回到安全点 (150, 0, 120)
 */
...
public:
    YoloGrabController(ros::NodeHandle& nh) : nh_(nh), detection_received_(false), conf_threshold_(0.0f)
    {
        ...
        // ========== split.py 启动命令 ==========
        nh_.param("split_launch_cmd", split_launch_cmd_,
                  std::string("rosrun yolo_Grab_pkg split.py"));
        ...
        ROS_INFO("  split_launch:    %s", split_launch_cmd_.c_str());
    }
    ...
    // ======================================================================
    // 启动 split.py (YOLO 检测节点)
    // ======================================================================
    void launchSplit()
    {
        ROS_INFO("===== Launching split.py: %s =====", split_launch_cmd_.c_str());
        int ret = std::system(split_launch_cmd_.c_str());
        if (ret != 0)
            ROS_WARN("split.py may have exited with code %d (might be normal)", ret);
        else
            ROS_INFO("split.py launched successfully");
    }
    ...
    void run()
    {
        ...
        // ===== Step 1: 启动 YOLO 检测节点 (split.py) =====
        ROS_INFO("===== Step 1: Launch YOLO detection node (split.py) =====");
        launchSplit();

        // ===== Step 2: 等待 YOLO 检测结果 =====
        ROS_INFO("===== Step 2: Wait for YOLO detection =====");
        if (!waitForDetection())
        {
            safeRetract();
            return;
        ...
        // ===== Step 3: 获取物块在 Base 坐标系下的位姿 =====
        ROS_INFO("===== Step 3: Get object pose in %s frame =====",
                 parent_frame_.c_str());
        ...
        // ===== Step 4: 移动到物块上方 (保持观测高度 120mm 水平平移) =====
        // 避免使用 PnP 解算的 Z (可能为负值), 以安全高度先到达物块正上方
        float approach_z = safe_z_;  // 沿用观测高度作为安全接近高度
        ROS_INFO("===== Step 4: Approach above object → (%.2f, %.2f, %.2f) =====",
        ...
        // ===== Step 5: 下降至抓取高度 =====
        float grasp_z = 30.0f;
        ROS_INFO("===== Step 5: Descend to grasp → (%.2f, %.2f, %.2f) =====",
                 obj_x, obj_y, grasp_z);
        ...
        // ===== Step 6: 开启吸盘 =====
        ROS_INFO("===== Step 6: Activate suction =====");
        setPump(true);
        ros::Duration(0.5).sleep();

        // ===== Step 7: 垂直上升 (抓取后抬起) =====
        float lift_z = grasp_z + lift_offset_;
        ROS_INFO("===== Step 7: Lift vertically → (%.2f, %.2f, %.2f) =====",
                 obj_x, obj_y, lift_z);
        ...
        {
            // 出刀点1 空闲 → 使用点1
            ROS_INFO("===== Step 8: Place at point 1 → (%.0f, %.0f, %.0f) =====",
        ...
        {
            // 出刀点1 已占用，点2 空闲 → 使用点2
            ROS_INFO("===== Step 8: Point 1 occupied, place at point 2 → (%.0f, %.0f, %.0f) =====",
        ...
        // ===== Step 9: 返回安全点 (150, 0, 120) =====
        ROS_INFO("===== Step 9: Return to safe point (150, 0, 120) =====");
